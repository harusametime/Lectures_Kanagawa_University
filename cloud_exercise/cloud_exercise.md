...
編集画面を開くと何も書かれていない空のエディタが開くので、下の**バケットポリシーのテンプレート**をまずコピーします。
...
```
その後、Resource の "(バケットARN)" のところに、編集画面の上部にあるバケット ARN をコピーして貼り付け、その後に `/*` を追加してください。Resource の内容は例えば以下のようになります。
 `test-samejima` は自分のバケット名に置き換えてください。
...
これで`ディストリビューションドメイン名/ファイル名`、例えば、`abcd1234.cloudfront.net/test.html`でファイルにアクセスすることが可能になります。
もし、ルート `abcd1234.cloudfront.net/` にアクセスしたときに、`test.html` を表示させたい場合は、ルートオブジェクトの設定が必要です。
